...
- [ ] **5.UV.3 — Adopt `EXPERIMENTAL_WebAuthNPluginPerformUserVerification2` in sidecar.** Add `WebauthnPluginUserVerificationRequest2` (48 bytes) to `types.rs`; triple-fallback entrypoint lookup (`_2` → `EXPERIMENTAL_WebAuthNPluginPerformUserVerification2` → v1); sidecar computes `pbBufferToSign = SHA-256(pbEncodedRequest)`; fallback-happened flag wired through IPC for the plugin-side dialog. No plugin-side UV verification yet. Candidate for Sonnet teammate dispatch (pure Rust, matches RS256 dispatch pattern).
