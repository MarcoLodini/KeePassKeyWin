...
- [ ] **5.UV.5 — Remove sidecar-side verification.** Delete the `verify_request_signature` gate in `src/KeePassKeyWin.Provider/src/com/server.rs` (keep the pubkey-fetch code in `request_sig.rs` — plugin still sources the pubkey from the sidecar via the hello extension from 5.UV.1). Plugin becomes the sole verifier for both signatures. Decide what to do with the now-dual `KEEPASSKEYWIN_SKIP_REQUEST_SIG_VERIFY` (sidecar-side) vs `KEEPASSKEYWIN_SKIP_PLUGIN_SIG_VERIFY` (plugin-side) — likely retire the sidecar one. Live-validate to confirm no regression.
