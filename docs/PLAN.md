...
- [ ] **5.UV.2 — Migrate `pbRequestSignature` verification plugin-side.** Sidecar forwards raw `pbRequestSignature` + `pbEncodedRequest` over IPC (new fields, optional for backward-compat); plugin re-verifies before acting using `EcdsaVerifier` + `OpSignPubKeyCache.Current`; sidecar-side gate kept as belt-and-braces for this sub-phase. Bypass via `BypassEnvVars.SkipPluginSigVerify`. **Decide here:** policy when `OpSignPubKeyCache.Current` is null at verification time — fail-open (skip verify, log loudly) or fail-closed (reject). Live-validate at webauthn.io. Root-session + Marco.
