...
- [x] **5.UV.4 — Plugin-side UV verification + fallback-warning dialog.** Shipped: TBD (Sonnet teammate dispatch, 2026-04-25). `UvFallbackPrompt` Core class with cached-decision latch (concurrent v1 ops block on first decision). `EcdsaVerifier.VerifyAcceptingEitherFormat` added — P1363 fast path (64-byte) + manual DER ECDSA-Sig-Value fallback (net48: manual SEQUENCE parser; net8+: reuses Verify+TryDerToP1363 helper). `VaultHandler` ctor extended with optional `UvFallbackPrompt?`; `VerifyUvSignatureOrPrompt` helper wires the full branch table: v2_stable/v2_experimental → ECDSA verify; v1/null/"" → prompt; unknown → fail-closed. Both `HandleMakeCredentialRaw` and `HandleGetAssertionRaw` call the helper. Plugin (`KeePassKeyWinExt.cs`) wires `MessageBox.Show`-based UV dialog (`#if NET48`; synchronous Invoke; default-button-No). `opSignPublicKeyB64` tightened to required in `HandshakeHandler.cs` and in Rust `ipc/mod.rs` `handshake()` (returns `Err` if `None` passed; field type changed from `Option<String>` to `String`, `skip_serializing_if` dropped). 37 new C# tests (17 matrix cases × 2 handlers, 3 new handshake tests, 1 JsonRpcFraming fix); 61 Rust tests pass. **Live validation still pending** — validate at webauthn.io before 5.UV.5 (removing sidecar gate).
