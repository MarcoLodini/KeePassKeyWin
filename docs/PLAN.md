...
- [x] **5.UV.3 — Adopt `EXPERIMENTAL_WebAuthNPluginPerformUserVerification2` in sidecar.** Shipped: TBD (Sonnet teammate dispatch, 2026-04-24; merge SHA pending root-session post-merge). `WebauthnPluginUserVerificationRequest2` (48 bytes, 6 ABI tests) added to `types.rs`. `webauthn_ext.rs` extended with `UvTier` enum, `PfnPerformUv2`, `perform_uv_v2: Option<PfnPerformUv2>`, `perform_uv_tier`, and `resolved_perform_uv_symbol_name()` diagnostic fn; triple-fallback lookup: `WebAuthNPluginPerformUserVerification2` → `EXPERIMENTAL_WebAuthNPluginPerformUserVerification2` → `WebAuthNPluginPerformUserVerification` (v1). SHA-256 helper: sidecar computes `sha2::Sha256::digest(cbor_bytes)` as `buffer_to_sign` (NIST known-vector test pinned in `request_sig.rs`). `server.rs` swaps to `perform_user_verification_2`, captures UV response bytes before freeing, extends IPC `params` with `uvSignatureB64` (base64-std of opaque PerformUV response) and `uvBindingTier` (`"v2_stable" | "v2_experimental" | "v1"`). Plugin (`VaultHandler.cs`) reads and logs both optional fields; absent = tolerated (pre-5.UV.3 sidecar). 4 new C# acceptance tests; 290/290 pass. **Live validation still pending** — sidecar belt-and-braces gate intact; no behaviour change for valid traffic. v2 entrypoint will be exercised for the first time during 5.UV.4 live-validation on Windows.
- [ ] **5.UV.4 — Plugin-side UV verification + fallback-warning dialog.** Plugin verifies UV signature against the op-sign pubkey cache from 5.UV.1; fail-closed on verify failure (no dialog, straight reject — MITM or Windows bug). On v1-fallback only: KeePass-owned confirmation dialog — *"UV signature verification unavailable on this Windows build. Proceed?"* with a details expander showing `GetProcAddress` result / HRESULT; decision cached per plugin-process lifetime (resets on KeePass restart so a swapped binary re-prompts). Tighten `opSignPublicKeyB64` from optional → required in hello. Live-validate. Root-session + Marco.
