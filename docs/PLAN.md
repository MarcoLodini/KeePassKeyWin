...
- [ ] **5.UV.6 — Docs + audit polish.** New "Trust boundaries and signature verification" section in `docs/ARCHITECTURE.md`; `docs/IPC_PROTOCOL.md` updated with new fields and the pubkey-fetch method; explicit trust-model section in `SECURITY.md` calling out what the plugin verifies independently vs what it sources from the sidecar. `CHANGELOG.md` entry. **Also rolls in three follow-ups from 5.UV.4.5/5.UV.7:** (a) plugin-side PII tier — `KeePassKeyWin.Core.Diagnostics.TraceLogger` is currently severity-less, so `VaultHandler.cs` call sites that interpolate `req.RpId` and `selected.UserName` (~lines 253, 274, 304) write unconditionally when `KEEPASSKEYWIN_LOG_FILE_PLUGIN` is set, parallel to the sidecar PII concern fixed in 5.UV.4.5; add a level parameter (Info/Debug) and gate user-supplied identifiers behind Debug. (b) Capture `from_env_lossy` per-directive parse warnings to the file route — currently they hit closed stderr in the COM-activated path, so a typo in `RUST_LOG` keeps the parseable rest but the user gets no signal which directive was rejected. (c) Delete `src/KeePassKeyWin.Provider/src/com/mod.rs` — it is dead code (the canonical `com` module is declared inline in `src/lib.rs` via `pub mod com { ... }`, so `mod.rs` is silently ignored by the Rust resolver). 5.UV.7 silent-failure-hunter flagged this as a maintenance trap; an annotated comment was added pointing to lib.rs as canonical, but removing the file is the cleanup.
