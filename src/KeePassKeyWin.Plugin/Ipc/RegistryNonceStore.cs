...
namespace KeePassKeyWin.Plugin.Ipc
{
    /// <summary>
    /// HKCU-backed single-use nonce for the keepasskeywin.hello handshake.
    ///
    /// Registry path: HKCU\Software\KeePassKeyWin\HandshakeNonce (REG_SZ, hex-encoded 32 bytes).
    /// Written at plugin startup; consumed (deleted) on first successful hello.
    /// Windows-only — call sites are guarded by OS-version check in KeePassKeyWinExt.
    /// </summary>
#if NET5_0_OR_GREATER
    ...
    {
        private const string RegPath = @"Software\KeePassKeyWin";
