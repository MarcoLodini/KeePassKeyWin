...
{
    // Diag breadcrumbs live flat under HKCU\Software\PassKee alongside the nonce
    // so the validator's existing `Get-ItemProperty HKCU:\Software\PassKee` dump
    // picks them up without needing to recurse into subkeys.
    private const string DiagRegPath   = @"Software\PassKee";
    ...
    /// HKCU\Software\PassKee\Diag\* — the nonce-never-appears symptom otherwise
    ...
    public override bool Initialize(IPluginHost host)
    {
        ...
        // If HKCU\Software\PassKee\Diag is missing after a failed run, we know
