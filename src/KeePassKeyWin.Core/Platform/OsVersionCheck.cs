...
namespace KeePassKeyWin.Core.Platform
{
    /// <summary>
    ...
    public static class OsVersionCheck
    {
        ...
        public static (bool Ok, string Reason) IsSupportedWindows()
        {
            if (!RuntimeInformation.IsOSPlatform(OSPlatform.Windows))
                return (false, $"KeePassKeyWin requires Windows 11 24H2 build {MinBuild}.{MinRevision} or newer. Non-Windows OS detected.");

            var v = GetWindowsVersion();
            if (v == null)
                return (false, $"KeePassKeyWin requires Windows 11 24H2 build {MinBuild}.{MinRevision} or newer. Could not determine OS version.");
        ...
        internal static (bool Ok, string Reason) CheckVersion(int major, int build, int ubr)
        {
            ...
            if (major < MinMajor || build < MinBuild || ubrFailsMinimum)
            {
                ...
                return (false,
                    $"KeePassKeyWin requires Windows 11 24H2 build {MinBuild}.{MinRevision} or newer. " +
                    $"Detected: {detected}");
            }
