# KeePassKeyWin Provider — Build Guide
...
Outputs:
- `target/x86_64-pc-windows-msvc/release/keepasskeywin_provider.dll`  — COM in-proc server
- `target/x86_64-pc-windows-msvc/release/keepasskeywin-provider.exe` — CLI smoke-test tool

### 3. Verifying the DLL

```bash
file target/x86_64-pc-windows-msvc/release/keepasskeywin_provider.dll
...
placeholders in `appx/Package.appxmanifest` are pre-filled for the KeePassKeyWin
dev cert (`CN=Marco Lodini, O=KeePassKeyWin, C=IT`, Version `0.0.1.0`); see
...
Outputs land in `out\` (gitignored): `KeePassKeyWin.Provider.msix`, `KeePassKeyWin.Dev.pfx`,
`cert-thumbprint.txt`.
