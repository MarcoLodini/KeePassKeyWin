//! PassKee's CTAP2 `authenticatorGetInfo` blob + AAGUID constant.
//!
...
pub const PASSKEE_AAGUID: [u8; 16] = [
...
///   3 (aaguid):      PASSKEE_AAGUID (non-zero, v4 UUID bytes)
...
pub fn authenticator_get_info_cbor() -> Vec<u8> {
    ...
    let aaguid   = Value::Bytes(PASSKEE_AAGUID.to_vec());
...
mod tests {
    ...
    fn aaguid_is_not_all_zeros() {
        ...
        assert!(
            PASSKEE_AAGUID.iter().any(|&b| b != 0),
            "AAGUID must not be all zeros — API rejects with 0x80090027",
        );
        assert_eq!(PASSKEE_AAGUID.len(), 16);
    }
    ...
    fn aaguid_key_roundtrips() {
        ...
        assert_eq!(aaguid.as_slice(), &PASSKEE_AAGUID);
    }
