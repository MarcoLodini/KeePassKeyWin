...
{
    /// <summary>
    /// Tests for the passkee.getAssertionRaw JSON-RPC method (Phase 4, CTAP 2.1 §6.2).
    /// </summary>
    public class GetAssertionRawTests
    {
        ...
        {
            var result = handler.Handle("passkee.getAssertionRaw", new JObject
            {
        ...
        public void DispatchTable_GetAssertionRaw_IsRouted()
        {
            var handler = MakeHandler(out _);
            var ex = Assert.Throws<RpcException>(() =>
                handler.Handle("passkee.getAssertionRaw", new JObject
                {
        ...
        public void MakeCredentialThenGetAssertion_RoundTripECDSASignatureVerifies()
        {
            ...
            var mcResult = (JObject)handler.Handle("passkee.makeCredentialRaw", new JObject
            {
        ...
        public void MakeCredential_ExtendedResponse_IncludesRpAndUserFields()
        {
            ...
            var result = (JObject)handler.Handle("passkee.makeCredentialRaw", new JObject
            {
